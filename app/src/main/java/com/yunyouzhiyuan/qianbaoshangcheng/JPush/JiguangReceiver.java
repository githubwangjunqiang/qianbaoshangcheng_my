...
public class JiguangReceiver extends BroadcastReceiver {
    ...
    private void toMainActivity(Context context) {
        ...
        //判断app进程是否存活
        if(systemUtils.isAppRunning(context, systemUtils.getCurrentProcessName(context))){
            Log.i(TAG, "toMainActivity: 存活");
            if(systemUtils.isAppRunningOnTop(context,systemUtils.getCurrentProcessName(context))){//是否在前台
                Log.i(TAG, "toMainActivity: 前台");

                Intent mainIntent = new Intent(context, DingdanWaimaiActivity.class);
                mainIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP);
                context.startActivity(mainIntent);

            }else{//在后台哦
                Log.i(TAG, "toMainActivity: 后台");
                Intent mainIntent = new Intent(context, DingdanWaimaiActivity.class);
                mainIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP);
                context.startActivity(mainIntent);
            }
        }else {
            Log.i(TAG, "toMainActivity: 没存活");
